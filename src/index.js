...
module.exports = {
	...
	methods: {
		...
		createRoute(opts) {
			...
			if (opts.tenancy) {
				let fn = this.tenancy;
				if (_.isString(opts.tenancy)) fn = this[opts.tenancy];

				if (!_.isFunction(fn)) {
					this.logger.warn(
						"Define 'tenancy' method in the service to enable multiple tenancy."
					);
					route.tenanc = null;
				} else route.tenancy = fn;
			}
			...
			if (rateLimit) {
				...
				else route.rateLimit.store = new MemoryStore(opts.window, opts);
			}
